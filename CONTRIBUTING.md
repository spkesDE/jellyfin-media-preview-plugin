...
A normal push to GitHub does not create a release.

The release workflow only runs when a tag matching `release-v*` is pushed, for example:
...
Releases are prepared locally and published through GitHub Actions.

1. update the plugin version in [Jellyfin.Plugin.MediaPreview.csproj](/C:/Users/spkes/Documents/Jellyfin%20Web%20hover-scrub/Jellyfin.Plugin.MediaPreview/Jellyfin.Plugin.MediaPreview.csproj)
2. commit your changes normally
3. optionally test the release ZIP locally with `.\build-release.ps1`
4. run:

```powershell
.\push-release.ps1 -ManifestChangelog "Short release summary for Jellyfin manifest"
```
...
- creates and pushes a `release-v...` tag
- triggers GitHub Actions

What GitHub Actions does:

- builds the plugin ZIP again on GitHub
...
- `ManifestChangelog` is only for the short text inside `manifest.json`
- `CHANGELOG.md` and the GitHub release notes come from the commit history
